...
def main(args):
    if args.eval_dataset == "nyu":
        depth_pathes = glob.glob("/mnt/disk5/data/eval/nyu_v2/val/nyu_depths/*.npy")
        ...
        if full:
            depth_pathes = glob.glob(f"/mnt/disk5/data/eval/sintel/training/depth/*/*.dpt")
            depth_pathes = sorted(depth_pathes)
        else:
            seq_list = [
                "alley_2",
            ...
            depth_pathes_folder = [
                f"/mnt/disk5/data/eval/sintel/training/depth/{seq}" for seq in seq_list
            ]
            depth_pathes = []
        ...
        img_pathes_folder = [
            f"/mnt/disk5/data/eval/bonn/rgbd_bonn_dataset/rgbd_bonn_{seq}/rgb_110/*.png"
            for seq in seq_list
        ]
        img_pathes = []
        ...
        depth_pathes_folder = [
            f"/mnt/disk5/data/eval/bonn/rgbd_bonn_dataset/rgbd_bonn_{seq}/depth_110/*.png"
            for seq in seq_list
        ]
        depth_pathes = []
        for depth_pathes_folder_i in depth_pathes_folder:
        ...
        depth_pathes = glob.glob(
            "/mnt/disk6/datasets/eval/kitti/depth_selection/val_selection_cropped/groundtruth_depth_gathered/*/*.png"
        )
        depth_pathes = sorted(depth_pathes)
        pred_pathes = glob.glob(
